...
class ModelTrainerPipeline:
    ...
    def run(self):
        ...
        try:
            ...
            config_manager = ConfigurationManager()
            model_training_config = config_manager.get_model_training_config()
            data_manager = DataManager()  # Create instance of DataManager

            pipeline_data = {
                'model_training_config': model_training_config,
                'data_manager': data_manager,  # Pass the instance, not the class
            }

            # Execute each step with the current pipeline_data
            for step in self.steps:
                result = step.execute(**pipeline_data)
                # Update pipeline_data with the results from this step
...
def create_model_training_step(name: str, mode: str) -> PipelineStep:
    ...
    - PipelineStep: An instance of PipelineStep configured for model training or sweeping.
    """
    def step_function(**kwargs):  # Accept all keyword arguments
        logger.info("Initializing model training")
        model_trainer = ModelTrainer(config=kwargs['model_training_config'])

        if mode == "train":
            logger.info("Training model...")
            model = model_trainer._train(
                kwargs['X_train_transformed'], 
                kwargs['y_train']
            )
            logger.info("Model training completed successfully.")
            return {'model': model}
            ...
            model_trainer.train_with_sweep(
                kwargs['X_train_transformed'],
                kwargs['y_train'],
                kwargs['X_val_transformed'],
                kwargs['y_val'],
                kwargs['model_training_config'].wandb_config.get("sweep", {}),
            )
            logger.info("Hyperparameter search completed successfully.")
            return {}
        else:
